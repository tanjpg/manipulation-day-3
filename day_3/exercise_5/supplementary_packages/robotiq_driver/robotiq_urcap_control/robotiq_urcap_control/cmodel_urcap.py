...
# Controlling the gripper directly through the Gripper URcap server.
...
class RobotiqCModelURCap:
    ...
    # ADDITIONAL EPICK WRITE VARIABLES (CAN ALSO READ)
    MOD = 'MOD'  # mod : ePick gripper mode (automatic(0) or advanced(1). Advanced mode requires pos, spe & for variables to be set)
    ...
    def connect(self, hostname, port = 63352, socket_timeout = 2.0):
        ...
        # Testing move and wait function
        #self.move_and_wait_for_pos(100, 50, 10)
        #self.move_and_wait_for_pos(150, 50, 100)
    ...
    def sendCommand(self, command):
        ...
        # Robotiq 2f
        if command.configuration != 2:
            scaled_position =  self.scale(command)
            print(int(scaled_position))
            self.move(int(scaled_position), command.rsp, command.rfr)
        # ePick
        else:
            self._set_var(self.MOD, 1) # Set advanced mode
            self.move(command.rpr, command.rsp, command.rfr) # Refer to example client for example

    def scale(self, command):
        # 255 = closed, 0 = open

        # https://stats.stackexchange.com/questions/281162/scale-a-number-between-a-range
        scaled_position2 = 255 - ((command.rpr - 0) / (command.robotiq2f_type - 0)) * (255 - 0)
        print(int(scaled_position2))
        # Percentage of width / robotiq_gripper width * 255.
        scaled_position = 255 - (255 * (command.rpr / command.robotiq2f_type))
        print(int(scaled_position))
    ...
    def getStatus(self):
        ...
        ## ePick variables 
        message.gmod = self._get_var(self.MOD) # ePick mode automatic(0)[NOT USED], advanced(1)
    ...
    # def activate(self, auto_calibrate): ## (Temporarily commented KHAIRUL)
    def activate(self):
        ...
        while not self.is_active():
            time.sleep(0.001)
        print("Activated.")
        # auto-calibrate position range if desired. (Temporarily commented KHAIRUL) 
        # if auto_calibrate:
        #     self.auto_calibrate()
...
#     gripper = Gripper2f85()
#     gripper.connect('192.168.1.10', 63352)
#     # gripper.activate()
